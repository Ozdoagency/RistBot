...
bot.onText(/\/start/, async (msg) => {
  ...
  const welcomeMessage = `Здравствуйте, ${firstName}! 👋 Меня зовут Виктория, я представляю онлайн-школу 'Rist'. Мы рады, что вы выбрали нас! Чтобы подобрать время для бесплатных пробных уроков, мне нужно задать пару вопросов. Расскажите, пожалуйста, какую цель вы хотите достичь с помощью занятий? Например, устранить пробелы, повысить оценки или подгото��иться к экзаменам. 🎯`;
...
bot.on('message', async (msg) => {
  ...
  try {
    ...
    if (userStages[chatId] < dialogStages.questions.length) {
      ...
      await sendTypingMessage(chatId, "Сейчас уточню доступное время у администратора и подтвержу выбранное время. Это зай��ет пару минут, ожидайте пожалуйста 😊");
