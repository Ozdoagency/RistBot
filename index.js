...
bot.onText(/\/start/, async (msg) => {
  ...
  const welcomeMessage = `Здравствуйте, ${firstName}! 👋 Меня зовут Виктория, я представляю онлайн-школу 'Rist'. Мы рады, что вы выбрали нас! Чтобы подобрать время для пробных уроков, мне нужно задать пару вопросов. Расскажите, пожалуйста, какие конкретные пробелы вы хотите устранить и в какой области? Например, устранить пробелы в знаниях, повысить оценки или что-то другое. 🎯`;
