...
function generatePrompt(userMessage, chatId) {
  const userHistory = userHistories[chatId] || [];
  const context = userHistory.map(entry => `Пользоват��ль: ${entry.response}\нИИ: ${entry.reply}`).join('\н');
...
bot.onText(/\/start/, async (msg) => {
  ...
  // Сбрасываем данные польз��вателя
  userHistories[chatId] = [];
  userRequestTimestamps[chatId] = { count: 0, timestamp: 0 };
  userStages[chatId] = 0; // Устанавливаем начальный этап

  const firstName = msg.from.first_name || 'пользователь';
  const welcomeMessage = `Здравствуйте, ${firstName}! 👋 Меня зовут Виктория, я представляю онлайн-школу 'Rist'. Мы ра��ы, что вы выбрали нас! Чтобы подобрать время для пробных уроков, мне нужно задать пару вопросов. 
...
bot.on('message', async (msg) => {
  ...
  try {
    ...
    if (userStages[chatId] < dialogStages.questions.length) {
      ...
      await sendTypingMessage(chatId, "Сейчас уточню доступное время у администратора и подтвержу выбранное время. Это займет пару минут, ожидай��е пожалуйста 😊");
